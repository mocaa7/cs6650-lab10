 ...
 */
public class _unused_BullyAlgoOld implements Runnable {
    ...
    public _unused_BullyAlgoOld(String operation) {
        this.operation = operation;

    }

    public _unused_BullyAlgoOld(String operation, String reqtype) {
        this.operation = operation;
    ...
    public static void main(String args[]) throws UnknownHostException, IOException, InterruptedException {
        initialize();
        Runnable receiver = new _unused_BullyAlgoOld("receiver");
        new Thread(receiver).start();

        // Thread.sleep(10);

        Runnable heartbeat = new _unused_BullyAlgoOld("heartbeat");
     ...
     * timer and heartbeat thread. The timer thread waits for 7 seconds to receive a
     * response. If it receives an OK but doesn't receive a leader then it starts an
     * election process again. The receiver thread accepts the various incoming
     * requests.
     *
     */
    public void run() {
        ...
        if (operation.equals("timer")) {
            ...
            try {
                ...
                if (!received) {
                    ...
                    Runnable sender = new _unused_BullyAlgoOld("sender", "cood");
                ...
                if (received && !leader_flag) {
                    ...
                    Runnable sender = new _unused_BullyAlgoOld("sender", "elecreq");
                    new Thread(sender).start();
                ...
                while (true) {
                    ...
                    if (option.equals("elecreq")) {
                        ...
                        if (self_id > source_id) {
                            Runnable sender = new _unused_BullyAlgoOld("sender", "ok");
                            new Thread(sender).start();
                        ...
                        if (!election_req) {
                            ...
                            Runnable timer = new _unused_BullyAlgoOld("timer");
                        ...
                        election_req = false;
                        received = false;
                        System.out.println("******LEADER selected is :" + leader_id + " : " + processes.get(leader_id)
                                + " Leader Flag= " + leader_flag);
                        System.out.println(
                                "Election Request value=  " + election_req + " Value of received= " + received);
    ...
    /**
     * The sendCoordinatorMsg() method broadcasts the leader to all the process. If
     * the process has failed then a message is displayed to indicate that the
     * process has .failed.
     *
     */
        ...
        for (int key : processes.keySet()) {
            if (key > self_id) {
                ...
                try {
                    ...
                    System.out.println(
                            "The process :" + destination_server + " has FAILED, cannot send Election Request !");
                    failure++;
                }
            }
